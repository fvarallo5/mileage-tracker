    ...
    return Consumer<AppState>(
      builder: (context, state, _) {
        ...
        return AppBottomSheet(
          title: 'Mileage Tracker Premium',
          subtitle: 'Drive hands-free. Log every mile.',
          children: [
            const _FeatureRow(
              icon: Icons.gps_fixed_rounded,
              title: 'Background GPS',
              subtitle: 'Keep tracking when you switch to Uber, DoorDash, or Maps.',
            ),
            const SizedBox(height: AppSpacing.md),
            const _FeatureRow(
              icon: Icons.radar_rounded,
              title: 'Auto-detect trips',
              subtitle: 'Automatically start and save trips when you begin driving.',
            ),
            const SizedBox(height: AppSpacing.md),
            const _FeatureRow(
              icon: Icons.shield_outlined,
              title: 'Privacy-first location',
              subtitle: 'Location is used only for trips. Clear disclosure before any background access.',
            ),
              ...
              child: Column(
                children: [
                  Text(
                    ...
                    style: const TextStyle(fontSize: 28, fontWeight: FontWeight.w800, color: AppColors.amber),
                  ),
                  const SizedBox(height: 4),
                  const Text('Cancel anytime', style: TextStyle(color: AppColors.textMuted, fontSize: 12)),
                ],
              ),
            ),
            ...
            if (kDebugMode) ...[
              ...
              Text(
                billing.storeAvailable
                    ...
                    : 'Store unavailable here — use the debug unlock to test Premium locally.',
                textAlign: TextAlign.center,
